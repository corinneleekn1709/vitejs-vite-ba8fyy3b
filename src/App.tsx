...
function AnnotationEditor({ imageSrc, annotations, onChange, onClose, mode }) {
  ...
  const getH = () => {
    ...
    return img ? Math.round(CANVAS_W * img.naturalHeight / img.naturalWidth) : 600;
  };
  ...
  const draw = useCallback(() => {
    ...
    rects.forEach(r => {
      ...
      ctx.strokeRect(r.x*CANVAS_W, r.y*H, r.w*CANVAS_W, r.h*H);
    });
  }, [annotations, drawing, start, current, imgLoaded]);
  ...
  const onMD = e => { setDrawing(true); const p=getPos(e); setStart(p); setCurrent(p); };
  const onMM = e => { if(drawing) setCurrent(getPos(e)); };
  const onMU = e => {
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", height:"100vh", background:"#0f172a" }}>
      {/* Toolbar */}
      <div style={{ flexShrink:0, display:"flex", justifyContent:"space-between", alignItems:"center", background:"#1e293b", padding:"12px 16px", gap:8, flexWrap:"wrap" }}>
        <div>
          <span style={{ fontWeight:700, fontSize:14, color:"#fff" }}>Annotate Screenshot</span>
          <span style={{ fontSize:12, color:"#94a3b8", marginLeft:8 }}>Draw rectangles to highlight areas</span>
        </div>
        <div style={{ display:"flex", gap:8, alignItems:"center", flexWrap:"wrap" }}>
          <div style={{ display:"flex", alignItems:"center", gap:4, border:"1px solid #475569", borderRadius:8, padding:"4px 10px", background:"#0f172a" }}>
            <button onClick={() => setScale(s=>Math.max(s-0.25,0.25))} style={{ border:"none", background:"none", cursor:"pointer", fontSize:18, color:"#94a3b8", lineHeight:1 }}>−</button>
            <span onClick={() => setScale(1)} style={{ fontSize:12, color:"#cbd5e1", fontWeight:600, cursor:"pointer", minWidth:36, textAlign:"center" }}>{Math.round(scale*100)}%</span>
            <button onClick={() => setScale(s=>Math.min(s+0.25,3))} style={{ border:"none", background:"none", cursor:"pointer", fontSize:18, color:"#94a3b8", lineHeight:1 }}>+</button>
          </div>
          <button onClick={() => onChange(annotations.slice(0,-1))} disabled={!annotations.length}
            style={{ border:"1px solid #475569", background:"#0f172a", borderRadius:8, padding:"6px 12px", fontSize:12, color:annotations.length?"#cbd5e1":"#475569", cursor:annotations.length?"pointer":"default", fontWeight:600 }}>
            ↩ Undo
          </button>
          <button onClick={() => onChange([])} disabled={!annotations.length}
            style={{ border:"1px solid #7f1d1d", background:"#0f172a", borderRadius:8, padding:"6px 12px", fontSize:12, color:annotations.length?"#f87171":"#7f1d1d", cursor:annotations.length?"pointer":"default", fontWeight:600 }}>
            Clear
          </button>
          <button onClick={onClose}
            style={{ border:"none", background:"#f8fafc", borderRadius:8, padding:"6px 18px", fontSize:12, color:"#1e293b", cursor:"pointer", fontWeight:700 }}>
            Done ✓
          </button>
        </div>
      </div>
      ...
      <div style={{ flex:1, overflow:"auto", padding:20, display:"flex", justifyContent:"center", alignItems:"flex-start" }}>
        <canvas ref={canvasRef} width={CANVAS_W} height={H}
          onMouseDown={onMD} onMouseMove={onMM} onMouseUp={onMU}
          style={{ cursor:"crosshair", borderRadius:8, display:"block", flexShrink:0, width:scaledW+"px", height:scaledH+"px" }}
        />
      </div>
    </div>
  );
...
async function buildCanvas(cards, mode) {
  ...
  canvas.width = CW * SCALE; canvas.height = totalH * SCALE;
  const ctx = canvas.getContext("2d");
  ...
  for (let ci = 0; ci < cards.length; ci++) {
    ...
    if (card.title) {
      ctx.fillStyle="#1e293b"; ctx.font="bold 30px Arial, sans-serif"; ctx.textAlign="center";
      ctx.fillText(card.title, CW/2, y+38); y+=56;
    }
    ...
    for (let li = 0; li < 2; li++) {
      ...
      ctx.fillStyle="#f1f5f9"; rrectTop(ctx,lx,y,SLOT_W,LABEL_H,10); ctx.fill();
      ctx.fillStyle="#64748b"; ctx.font="bold 16px Arial, sans-serif"; ctx.textAlign="center";
      ctx.fillText(lbl, lx+SLOT_W/2, y+LABEL_H/2+6);
    }
    y += LABEL_H;

    const drawSlot = (img, isNoVersion, x, anns) => {
      ctx.save(); rrectBot(ctx,x,y,SLOT_W,imgH,10); ctx.clip();
    ...
    drawSlot(imgB, card.newIsNew, leftX, card.newAnnotations);
    drawSlot(imgA, card.oldIsNew, rightX, card.oldAnnotations);
    y += imgH;
        ...
        {modes.map(m => (
          <div key={m.id} onClick={() => onSelect(m.id)}
            ...
            style={{ width:220, borderRadius:16, overflow:"hidden", cursor:"pointer",
              ...
              border:"1.5px solid", borderColor:hovered===m.id?"#94a3b8":"#e2e8f0", display:"flex", flexDirection:"column" }}>
...
function AnnotatedImage({ src, annotations }) {
  ...
  const redraw = () => {
    ...
    (annotations||[]).forEach(r => {
      ctx.strokeStyle="#ef4444"; ctx.lineWidth=Math.max(2, img.naturalWidth*0.003);
...
// ── UploadBox ──────────────────────────────────────────────────────────────────
function UploadBox({ label, image, onUpload, isNoVersion, onToggleNoVersion, annotations, onAnnotationsChange, onAnnotate }) {
  ...
  return (
    <div style={{ flex:1, display:"flex", flexDirection:"column", gap:8 }}>
        ...
        <div style={{ display:"flex", background:"#f1f5f9", borderRadius:8, padding:3, gap:2, width:"100%" }}>
          {["upload","paste"].map(t => (
            <button key={t} onClick={() => setTab(t)} style={{
              flex:1, border:"none", borderRadius:6, padding:"8px 0", fontSize:13, fontWeight:600, cursor:"pointer",
              background:tab===t?"#fff":"transparent", color:tab===t?"#374151":"#94a3b8",
              boxShadow:tab===t?"0 1px 4px rgba(0,0,0,0.08)":"none",
              display:"flex", alignItems:"center", justifyContent:"center", gap:6,
            }}>
              <span>{t==="upload"?"📁":"📋"}</span><span>{t==="upload"?"Upload":"Paste"}</span>
            </button>
          ))}
        </div>
      )}
        ...
        <div style={{ position:"relative" }}>
          ...
          {image && (
            <button onClick={onAnnotate} style={{
              position:"absolute", bottom:8, right:8, border:"none", background:"#1e293b",
              borderRadius:7, padding:"6px 12px", fontSize:12, color:"#fff", cursor:"pointer", fontWeight:600,
      ...
      {!image&&(
        <button onClick={onToggleNoVersion} style={{ border:"1px solid #e2e8f0", background:isNoVersion?"#f1f5f9":"#fff", borderRadius:7, padding:"5px 0", fontSize:12, color:isNoVersion?"#475569":"#94a3b8", cursor:"pointer", fontWeight:500 }}>
          {isNoVersion?"↩ Upload screenshot instead":"✨ No previous version"}
        </button>
      )}
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
            ...
            <div style={{ display:"flex", gap:16 }}>
              <UploadBox label={newLabel||"Newer"} image={newImg} isNoVersion={newIsNew}
                ...
                onAnnotate={()=>onChange({_annotating:"new"})}
              ...
              <UploadBox label={oldLabel||"Older"} image={oldImg} isNoVersion={oldIsNew}
                ...
                onAnnotate={()=>onChange({_annotating:"old"})}
            ...
            <button disabled={!ready} onClick={()=>onChange({done:true})} style={{
              background:ready?"#1e293b":"#e2e8f0", color:ready?"#fff":"#94a3b8", border:"none", borderRadius:10,
...
export default function App() {
  ...
  // annotation state lifted to top level so it renders outside scroll containers
  const [annotatingCard, setAnnotatingCard] = useState(null); // { id, side }

  if (!mode) return <ModeSelector onSelect={m => { setMode(m); setCards([newCard(1)]); setPreviewUrl(null); }} />;

  const update = (id, patch) => {
    // intercept _annotating flag
    if (patch._annotating) {
      setAnnotatingCard({ id, side: patch._annotating });
      return;
    }
  ...
  // Find current annotation context
  ...
  // If annotating, show fullscreen editor instead of main UI
  if (annotatingCard && annotCard && annotImg) {
    return (
      <div style={{ fontFamily:"Inter,Arial,sans-serif", height:"100vh", overflow:"hidden" }}>
        <AnnotationEditor
          imageSrc={annotImg}
          annotations={annotAnns||[]}
          mode={mode}
          onChange={v => {
            const key = annotatingCard.side==="new" ? "newAnnotations" : "oldAnnotations";
            setCards(cs => cs.map(c => c.id===annotatingCard.id ? Object.assign({},c,{[key]:v}) : c));
          }}
  ...
  return (
    <div style={{ fontFamily:"Inter,Arial,sans-serif", maxWidth:1100, margin:"0 auto", padding:"32px 20px" }}>
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:24 }}>
        ...
        <div style={{ display:"flex", gap:12 }}>
          <button onClick={handlePreview} disabled={!doneCount||previewing||exporting} style={{
            ...
            cursor:doneCount&&!previewing&&!exporting?"pointer":"not-allowed",
          }}>
          ...
          <button onClick={handleExport} disabled={!doneCount||exporting||previewing} style={{
            ...
            fontSize:15, fontWeight:600, cursor:doneCount&&!exporting?"pointer":"not-allowed",
          }}>
      ...
      {previewUrl&&(
        <div onClick={()=>setPreviewUrl(null)} style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.7)", zIndex:1000, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"flex-start", padding:"16px", overflowY:"auto", cursor:"pointer" }}>
